...
var globe={"testslct":0,"del":0,"dupe":0,"reselect":0,"chord":[]}
var blue="#6ea4ff"
var pink="#ff70d4"
...
function checkChord() {
  var full=false
  var ic=[16,17,18] //interested chord keys:16 shift,17 ctrl, 18 alt;
  //.sort() is a mess of a function but luckily we only care about 16,17,18
  //  so for it works fine for our uses
  ...
  if ( globe["chord"].length==3 && checkChord() && kc == 82 ) { //82 : r 
    ...
    if ( txtchk ) {
      ...
      if ( ac == "why" ) {
        ...
        var aci=parseInt(acn.split(ac)[1])-1
        var txta=document.getElementById(id)
        var val=txta.value
        ...
        console.log("at:..",alltests[fn]["arguments"])
        console.log("aci..",aci)
        ...
        tst_onit()
        ted_onit()
        var func=alltests[fn]["funcname"]
        showTest(String(fn)+"."+func)
        }
      }
    }
    }
...
function showTest(ins) {
  ...
  var spl=ins.split(".")
  var tind=parseInt(spl[0])
  var fn=spl[1]
  globe["testslct"]=tind
  for ( var ti=0,tl=alltests[tind]["tests"].length; ti<tl;ti++) {
    var strti=String(ti)
    var did="tst"+String(ti)
    var style={"background":"#347"}
    var cont="createDiv_ui_tst"
    createDiv(ct,did,style,cont,[alltests,tind,ti,tl])
 }       
} //showTest(), display all the tests from a single function

function createDiv_ui_assert(uio,fulld,atst,args,tind,ti) {     
      var para={}
      para["cols0"]=34
      para["cols1"]=17
      para["cols"]=[34,17]
...
function createDiv_ui_tst(uio,alltests,tind,ti,tl) {
    ...
    var tt=alltests[tind]["tests"][ti]
    var tid="txta."+String(tind)+"."+String(ti)+".why"
    ...
    //imagine, createDiv() 
    var atst=alltests[tind]["tests"][ti]
    var args=alltests[tind]["arguments"]
    if ( args.length == 0) {
      var expt=document.createElement("p")
      expt.id="expt0"
      expt.innerHTML="empty function"
    } else {
      var did=String(tind)+"."+String(ti)+".assert"
      var style={"background":"#347"}
      var cont="createDiv_ui_assert"
      var uiargs=[fulld,atst,args,tind,ti] //arguments for the createDiv_ui_FUNC()
      createDiv(fulld,did,style,cont,uiargs)

      var id="txta."+String(tind)+"."+String(ti)+".expect"+String(1)
      ...
      var id=String(tind)+"."+String(ti)+".procd"+String(1)
      ...
      var did=String(tind)+"."+String(ti)+".sufa"+String(1)
      var bg=pink
